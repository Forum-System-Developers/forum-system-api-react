...
from forum_system_api.schemas.message import MessageCreate, MessageCreateByUsername, MessageResponse
...
async def create_message_by_username(
    ...
    receiver = user_service.get_by_username(username=message_data.receiver_username, db=db)
    if not receiver:
        raise HTTPException(
